...
const getCalendarBounds = () => {
  ...
  let endDate = startDate.clone().add(34, "days"); // 35日分の範囲を確保
...
// カレンダー作成
const getCalendar = () => {
  const { startDate, endDate } = getCalendarBounds();
  const days = [];
  ...
  // 最後の週に足りない日を追加して7日分にする
  const lastWeekDays = 7 - days.length % 7;
  if (lastWeekDays !== 7) {  // 余分な週を作らないために条件をチェック
      const lastDate = days[days.length - 1].date;
      const lastDayOfMonth = moment(lastDate).endOf('month');
      let nextDate = lastDate.clone().add(1, 'day');
      
      // 月を越す日を追加しない
      for (let i = 0; i < lastWeekDays; i++) {
          if (nextDate.isAfter(lastDayOfMonth)) break; // 月越しを避ける
          days.push({
              day: nextDate.date(),
              date: nextDate.clone(),
              dayEvents: []
